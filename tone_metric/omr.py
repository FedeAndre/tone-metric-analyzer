...
def pdf_to_annotations(pdf_path: str | Path, output_dir: str | Path) -> tuple[Path | None, str]:
    ...
    This is deliberately optional for display: if annotation export fails, canonical
    score-time analysis can still succeed, but the PDF overlay is reported unavailable.
    """
    pdf_path = Path(pdf_path); output_dir = Path(output_dir); output_dir.mkdir(parents=True, exist_ok=True)
    cmd = find_audiveris()
    if not cmd:
