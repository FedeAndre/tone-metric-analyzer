            ...
            for system_index, system in enumerate(systems):
                ...
                for stack_index, stack in enumerate(stacks):
                    ...
                    for cid in head_chords:
                        ...
                        if exact_head_geometry:
                            center = (sum(hcx) / len(hcx), sum(hcy) / len(hcy))
                        else:
                            center = (x + w / 2, y + h / 2)
                    ...
                    for group in clusters:
                        ...
                        attack_events = [e for e in head_events if e.get("attack")]
                        exact_attack_events = [e for e in attack_events if e.get("head_geometry_exact")]
                        rep_pool = exact_attack_events or attack_events or head_events or group
                        rep = min(
                            rep_pool,
                            key=lambda a: (
                                sum(abs(a["x_abs"] - b["x_abs"]) for b in rep_pool),
                                a["cid"],
                            ),
                        )
