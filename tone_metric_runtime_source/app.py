...
app = FastAPI(title="Tone-Metric Analyzer", version="0.15.3")
app.mount("/static", StaticFiles(directory=ROOT / "static"), name="static")

# Hosted PDF recognition can exceed a reverse-proxy request timeout.  Keep the
# validated analyzer unchanged, but run long analyses in a worker thread and
# let the browser poll short status requests.
JOBS: dict[str, dict] = {}
...
async def analyze_upload(file: UploadFile = File(...), meter_override: str = Form("auto")):
    ...
    try:
        ...
        analysis_hits = hits
        analysis_measures = measures
        analysis_symbolic_hits = hits
        ...
        if suffix == ".pdf" and omr_path is not None:
            try:
                analysis_measures, analysis_symbolic_hits, framework_warnings, measure_framework_meta = (
                    reconcile_measure_framework_from_omr(
                        omr_path, measures, symbolic_hits=hits, initial_meter_override=meter_override_tuple
                    )
                )
                ...
                recovered_hits, canonical_warnings, canonical_score_meta = build_hits_from_canonical_score(
                    omr_path, analysis_measures, symbolic_hits=analysis_symbolic_hits
                )
                if recovered_hits:
                    ...
                    # A restored measure framework is only useful when OMR attack
                    # recovery succeeds.  Preserve the previous symbolic fallback as
                    # one internally consistent MusicXML timeline.
                    analysis_hits = hits
                    measures = measures
            except Exception as exc:
        ...
        # v0.15.3 aligns the executable output with the mathematical paper by
        ...
        # Non-tuplet canonical timing and exact attack registration remain unchanged;
        # validated 3:2 tuplets are retained on an independent local ternary layer.
