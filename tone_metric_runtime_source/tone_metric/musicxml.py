...
def parse_musicxml(path: str | Path, initial_meter_override=None) -> Tuple[List[Hit], List[MeasureInfo], List[str]]:
    ...
    - grace notes: ignored
    - tuplets (time-modification): actual attack retained at its exact MusicXML score time
    - rests: no attack
...
def extract_visual_groups(path: str | Path, initial_meter_override=None) -> tuple[list[dict], bool]:
    ...
    those noteheads are physically present on the score. Tuplet attacks are now analysis
    hits and therefore remain marked as analyzed. Each group is marked
