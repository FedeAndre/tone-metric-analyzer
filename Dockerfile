...
RUN apt-get update && apt-get install -y --no-install-recommends     ca-certificates coreutils curl python3 python3-pip     tesseract-ocr tesseract-ocr-eng fontconfig fonts-dejavu-core fonts-liberation     libasound2t64 libgtk-3-0t64 libx11-6 libxext6 libxi6 libxrender1 libxtst6 xdg-utils  && (curl -fL --retry 8 --retry-delay 5 --retry-all-errors -o /tmp/audiveris.deb       "https://github.com/Audiveris/audiveris/releases/download/${AUDIVERIS_VERSION}/Audiveris-${AUDIVERIS_VERSION}-ubuntu24.04-x86_64.deb"      || curl -fL --retry 8 --retry-delay 5 --retry-all-errors -H 'Accept: application/octet-stream'       -o /tmp/audiveris.deb "https://api.github.com/repos/Audiveris/audiveris/releases/assets/${AUDIVERIS_ASSET_ID}")  && echo "${AUDIVERIS_SHA256}  /tmp/audiveris.deb" | sha256sum -c -  && dpkg-deb -x /tmp/audiveris.deb /  && test -x /opt/audiveris/bin/Audiveris  && rm -f /tmp/audiveris.deb && rm -rf /var/lib/apt/lists/*
WORKDIR /app
