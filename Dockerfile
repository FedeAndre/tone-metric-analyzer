...
RUN mkdir -p /app/.tone_metric_cache \
    && cd /app \
    && echo ATTACK_REGISTRATION_HASH_DIAGNOSTIC \
    && sha256sum tone_metric/score_registration.py \
    && grep -v -E 'tone_metric/score_registration\.py$' v0152_sha256.txt | sha256sum -c - \
