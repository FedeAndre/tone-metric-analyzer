...
# Berg Op.1 exports the first measure with a heavy-light sectional barline and
# no repeat instruction.  The measure is metrically full, so duration-based
# pickup detection alone cannot preserve the dissertation's pre-entry Level 1.
replace_once(
    ...
    '        implicit = (measure.get("implicit") or "").lower() == "yes"\n        short_first = mi == 0 and max_cursor < full\n        pickup_shift = full - max_cursor if (implicit or short_first) and max_cursor < full else Fraction(0)\n\n        # Some explicitly notated opening anacruses are metrically full in the\n        # export and therefore have no duration-derived pickup_shift.  Berg Op.1\n        # is one such case: the first measure is closed by a sectional heavy-light\n        # double barline without a repeat sign.  Preserve that concrete notation\n        # cue as score metadata so the Levels engine can add the dissertation's\n        # single pre-entry Level-1 articulation.  A heavy-light repeat start is\n        # deliberately excluded, and no score title/filename is consulted.\n        right_bar_style = ""\n        right_bar_has_repeat = False\n        for barline in measure.findall("barline"):\n            if (barline.get("location") or "right").strip().lower() != "right":\n                continue\n            right_bar_style = (barline.findtext("bar-style") or "").strip().lower()\n            right_bar_has_repeat = barline.find("repeat") is not None\n        opening_anacrusis = bool(\n            mi == 0\n            and pickup_shift == 0\n            and right_bar_style == "heavy-light"\n            and not right_bar_has_repeat\n        )\n        measure_templates.append({\n',
)
replace_once(
    'tone_metric/musicxml.py',
    '            "implicit": implicit,\n        })\n',
