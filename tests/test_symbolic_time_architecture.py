...
from tone_metric.omr_project import OmrSlot
from tone_metric.score_registration import build_layer_anchors_from_exact_omr_slots
...
<score-partwise version="4.0">
  ...
  <part id="P2"><measure number="1">
    ...
    def test_buxtehude_measure4_pattern_cannot_be_rewritten_by_geometry(self):
        ...
        hits = [self._hit(Fraction(0)), self._hit(Fraction(1)), self._hit(Fraction(2), 3), self._hit(Fraction(5, 2), 2), self._hit(Fraction(3))]
    ...
    def test_metric_grid_keeps_legitimate_empty_structural_positions(self):
        ...
        by_time = {p["time_quarter"]: p for p in result["segments"][0]["structural_points"]}
    ...
    def test_buxtehude_measure46_ten_ground_truth_onsets_remain_ten_events(self):
        ...
        times = [Fraction(0), Fraction(1, 2), Fraction(3, 4), Fraction(1), Fraction(3, 2), Fraction(7, 4), Fraction(2), Fraction(5, 2), Fraction(3), Fraction(7, 2)]
    ...
    def test_exact_omr_slot_registration_cannot_create_or_retime_events(self):
        measure = MeasureInfo(0, "4", Fraction(0), Fraction(4), Fraction(4), Fraction(0), 4, 4)
        result = analyze([self._hit(Fraction(0), 2), self._hit(Fraction(1))], [measure])
        slots = [
            OmrSlot(0, 0, 0, 0, "1", 100, 500, 1, Fraction(0), Fraction(0), 50, 150, False),
            OmrSlot(0, 0, 0, 0, "1", 100, 500, 2, Fraction(1, 4), Fraction(1), 180, 280, False),
            OmrSlot(0, 0, 0, 0, "1", 100, 500, 3, Fraction(3, 8), Fraction(3, 2), 240, 340, False),
        ]
        meta = {"pages": [{"page_index": 0, "width": 1000, "height": 1400, "system_bounds": []}]}
        anchors, _structural, stats, _warnings = build_layer_anchors_from_exact_omr_slots(result, slots, meta)
        mapped = [a for rows in anchors.values() for a in rows]
        self.assertEqual(len(mapped), 2)
        self.assertEqual({a["attack_key"] for a in mapped}, {"0:0", "0:1"})
        self.assertEqual({a["recovered_x_abs"] for a in mapped}, {150.0, 280.0})
        self.assertFalse(stats["registration_can_create_attacks"])
        self.assertFalse(stats["registration_can_retime_attacks"])
        self.assertFalse(stats["registration_can_nearest_match_time"])
        self.assertFalse(stats["musicxml_layout_coordinates_used"])
