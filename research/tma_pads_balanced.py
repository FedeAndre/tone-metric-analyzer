...
OUT=Path("research/tma_pads_balanced_results");OUT.mkdir(parents=True,exist_ok=True)
CACHE=Path("/tmp/pads_bal");CACHE.mkdir(exist_ok=True)
RNG=np.random.default_rng(20260925);N_PER=40
...
def main():
    ...
    summ={"dataset":"PADS balanced sensitivity","selected_per_group":N_PER,"n_analyzed":len(df),"groups":df.groupby("group").size().to_dict(),
